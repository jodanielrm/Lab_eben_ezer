...
Public Class Citologia
    ...
    Public Sub autonumresultados()
        query = "select Id_ResultadoC from Citologia"
        If con.val(query) = True Then
            query = "select Max(Id_ResultadoC) from Citologia"
            dr = con.reader(query)
            While dr.Read
    ...
    Private Sub BtnQuimicaG_Click(sender As Object, e As EventArgs) Handles BtnQuimicaG.Click
        ...
        Else
            Try
                query = "INSERT INTO Citologia VALUES(" & TxtIdResultado.Text & ", CONVERT (date, GETDATE())," & TxtId.Text & "," & TxtIdMedico.Text & ",7,'" & DtpToma.Text & "','" & DtpRecibo.Text & "','" & DtpInforme.Text & "','" & TxtAdecuada.Text & "','" & TxtInadecuada.Text & "','" & TxtEndo.Text & "','" & CmbNegativo.Text & "','" & CmbReaccion.Text & "','" & CmbFlora.Text & "','" & CmbOtro.Text & "','" & TxtNota.Text & "','" & TxtObservacionC.Text & "','" & CmbSugerencia.Text & "')"
                con.insertar(query)

                MsgBox("Examen Registrado con exito", MsgBoxStyle.Information)
                resultadocito = TxtIdResultado.Text
                autonumresultados()
                Dim ans As String

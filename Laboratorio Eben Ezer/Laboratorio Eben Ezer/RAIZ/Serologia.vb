...
Public Class Serologia
    ...
    Public Sub autonumresultados()
        query = "select Id_ResultadoS from Serologia"
        If con.val(query) = True Then
            query = "select Max(Id_ResultadoS) from Serologia"
            dr = con.reader(query)
            While dr.Read
    ...
    Private Sub Serologia_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        TxtFecha.Text = Today
    ...
    Private Sub BtnQuimicaG_Click(sender As Object, e As EventArgs) Handles BtnQuimicaG.Click
        ...
        Else
            Try
                query = "INSERT INTO Serologia VALUES(" & TxtIdResultado.Text & ", CONVERT (date, GETDATE())," & TxtId.Text & "," & TxtIdMedico.Text & ",8,'" & TxtASO.Text & "','" & TxtRPR.Text & "','" & TxtFR.Text & "','" & TxtPCR.Text & "','" & TxtVIH.Text & "','" & TxtHPS.Text & "')"
                con.insertar(query)

                MsgBox("Examen Registrado con exito", MsgBoxStyle.Information)
                resultadosero = TxtIdResultado.Text
                autonumresultados()
                Dim ans As String

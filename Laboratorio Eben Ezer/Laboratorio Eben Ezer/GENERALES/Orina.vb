...
Public Class Orina
    ...
    Public Sub autonumresultados()
        query = "select Id_ResultadoO from Orina"
        If con.val(query) = True Then
            query = "select Max(Id_ResultadoO) from Orina"
            dr = con.reader(query)
            While dr.Read
    ...
    Private Sub Orina_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub BtnQuimicaG_Click(sender As Object, e As EventArgs) Handles BtnQuimicaG.Click
        ...
        Else
            Try
                query = "INSERT INTO Orina VALUES(" & TxtIdResultado.Text & ", CONVERT (date, GETDATE())," & TxtId.Text & "," & TxtIdMedico.Text & ",9,'" & CmbAspecto.Text & "','" & CmbColor.Text & "','" & CmbOlor.Text & "','" & CmbSedimento.Text & "','" & TxtDensidad.Text & "','" & TxtGlucosa.Text & "','" & TxtBilirrubina.Text & "','" & TxtCetona.Text & "','" & TxtPh.Text & "','" & TxtSo.Text & "','" & TxtProteina.Text & "','" & TxtUrobilinogeno.Text & "','" & TxtNitrito.Text & "','" & TxtLeucositos.Text & "','" & TxtCE.Text & "','" & TxtCeritrocitos.Text & "','" & TxtLeucositosM.Text & "','" & CmbBacteria.Text & "','" & CmbMocus.Text & "','" & TxtCristales.Text & "','" & TxtCilindros.Text & "','" & TxtComentarios.Text & "')"
                con.insertar(query)

                MsgBox("Examen Registrado con exito", MsgBoxStyle.Information)
                resultadoOrina = TxtIdResultado.Text
                autonumresultados()
                Dim ans As String

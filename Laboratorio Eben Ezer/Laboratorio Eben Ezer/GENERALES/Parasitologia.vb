...
Public Class Parasitologia
    ...
    Public Sub autonumresultados()
        query = "select Id_ResultadoP from Parasitologia"
        If con.val(query) = True Then
            query = "select Max(Id_ResultadoP) from Parasitologia"
            dr = con.reader(query)
            While dr.Read
    ...
    Public Sub cargar()
        ...
        End If
    End Sub
    Private Sub Parasitologia_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub BtnQuimicaG_Click(sender As Object, e As EventArgs) Handles BtnQuimicaG.Click
        ...
        Else
            Try
                query = "INSERT INTO Parasitologia VALUES(" & TxtIdResultado.Text & ", CONVERT (date, GETDATE())," & TxtId.Text & "," & TxtIdMedico.Text & ",10,'" & CmbColor.Text & "','" & CmbConsistencia.Text & "','" & CmbMoco.Text & "','" & CmbSM.Text & "','" & TxtTrofozoitos.Text & "','" & TxtQuistes.Text & "','" & TxtHuevos.Text & "','" & CmbLarvas.Text & "','" & CmbLeucocitos.Text & "','" & TxtEritrocitos.Text & "','" & TxtOtros.Text & "','" & TxtNota.Text & "')"
                con.insertar(query)

                MsgBox("Examen Registrado con exito", MsgBoxStyle.Information)
                resultadoParasito = TxtIdResultado.Text
                autonumresultados()
                Dim ans As String
